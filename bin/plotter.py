...
c = 0
p = 1
for files in listdir('ScalarFields'):
    filename = files.split('.')[0]
    c = int(filename.split("_")[1])

    
    data = np.genfromtxt('ScalarFields/' + filename + '.csv', delimiter=',')
    data = data.astype(float)

    fig = plt.figure()
    print("Plotting {:.2f}% ".format((p/ len(listdir('ScalarFields')))*100),end='\r',)
    ...
    plt.savefig('Frames/' + filename.split("_")[0] +"_" + str(int(c)) + '.png')
